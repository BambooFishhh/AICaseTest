...
### v8.9.7-v8.9.8 — litemall 全链路实测 + 12.11~12.14 修复落地

首次用真实商城 litemall（手机端 H5 商城，仅商城无管理后台）+ mimo-v2.5/qwen-embedding 打通「导入→范围→PRD→分析→生成→执行」全链路：
- **环境**：project 版 litemall（后端 jar :8080 + 前端 vite :6255，源码 `projects/litemall`，git 基线 master + feature 分支）；执行目标 URL 用稳定 Docker 桥 `172.31.160.1:6255`（WLAN IP 会变）。
- **临时适配**：浏览器移动模拟（`devices['iPhone 14']`，可回退桌面）+ `browser_set_storage` 注入 localStorage 登录态（litemall 为 token 型非 cookie）。
- **实测结论→任务卡**（§9.6）：12.9 回测（结构 1.0 / 召回 0.64 / 覆盖 0.45）、12.7 SSE 50 并发静默排队、12.8 票据互通但 SSE 实例绑定断连即取消。
- **12.13 live 口径**：expected.json 接口注入 BackendResult + requirementIdMap 召回 → 复测**接口覆盖 0.45→1.0**（large 覆盖=0 证实为口径假阳性）。
- **12.14 导航缺口**：执行器导航分支（route 判定+hash 兜底）+ frontendRoutes 注入 + prompt 导航首步规则 + 存量用例确定性兜底 → 重生成用例首步导航实测通过。
- **12.11 SSE 断连宽限 + 跨实例回放**：断连不再立即取消（宽限期）；生成期事件持久化 + `generate-stream-replay` 端点跨实例回放。
- **12.12 排队闸门 + 首事件**：生成队列预检闸门（超限快速 503）+ 即时 `started`/`queued` 事件 + 前端监听。

详见：`docs/CHANGELOG.md` v8.9.8 小节 + `docs/长期迭代计划书.md` §9.6/12.11~12.14
