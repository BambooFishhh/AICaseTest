...
**日期**: 2026-09-01
**基线**: v9.6
**主题**: `b9d451db` 批量 23 条 6 过/17 挂的根因修正——断言空白归一、手势步骤硬拒绝、假选择器真实池校验、22 条用例按真实 UI/数据修正。

- **引擎**：`ExecutionAssert` 页面文本与引号短语折叠空白（修复 `'user123 欢迎回来'`、`'0 待付款 1 待发货'` 因换行误判）；`ReviewQualityChecker.hasGestureViolation` + `ruleReview` 拒绝左滑/右滑/上滑/长按步骤填补 12.17 后置兜底；`enrichStructuredSteps` 对 LLM 自填 css/text 选择器按当前路由真实池校验，不在池内剔除并补真实选择器（消灭 `.collect-icon`/`.footprint-item .checkbox` 超时）。
- **用例（22 条）**：删除无真实 UI 语义的 TC-908；聚合/用户中心改真实文案锚点；收藏取消改详情页路径；足迹用例先打开 `/goods/1006002` 生成可见足迹再操作；未登录用例先点「退出登录」再断言跳转；商品/订单列表用真实商品名与 `订单号：` 断言。
- **数据侧**：目标商品固定为有效 ID 1006002（轻奢纯棉刺绣水洗四件套），避免应用侧系统内部错误。

**验证**：后端 565/565 绿（ExecutionAssert 换行回归 1 例、手势违规 1 例、假选择器剔除 1 例）；前端 `npm run build` 通过；批量重跑结果见部署后批次。
...
**日期**: 2026-08-26
**基线**: v8.9.4
**主题**: 计划书「阶段 6」任务 12.8 全量交付 + 12.7 首组实测数据。双实例演练三项核心机制实证通过（调度互斥/JWT 跨实例/DB 权威源互通）；短请求冒烟 300 请求零失败 P95=26ms。纯文档/工程版本，无代码变更

### 工程变更

| 文件 | 变更 | 说明 |
|---|---|---|
| `docs/水平扩容指南.md` | **新增** | **12.8** 拓扑图/多实例环境变量清单（APP_REDIS_ENABLED=true 为硬约束）/LLM 限流聚合公式（实例配额×实例数≤供应商RPM）/Nginx ip_hash vs 重连两方案对比与样例/**附录：双实例演练记录** |
| `perf/k6/short-requests.js` | **新增** | **12.7** k6 并发场景脚本（VU/时长可配；SSE 生成场景需 mock LLM 供应商 [需确认] 后补 sse-generation.js） |
| `perf/smoke-shortrequests.ps1` | **新增** | **12.7** PowerShell 顺序冒烟工具（无外部依赖；本机 k6 镜像拉取超时的降级方案） |
| `docs/容量基线报告.md` | **新增** | **12.7** 首组实测数字 + CR §9.4 理论对照 + 待补项清单 |

### 双实例演练记录（摘要）

第二实例同镜像入 compose 网络（:8001）：① 两实例 health UP 且 B 的 dataSource/milvus 正常；② A 登录 JWT 在 B 直接可用；③ B 创建项目 A 立即可见（DB 权威源）；④ **ShedLock 实证**——shedlock 表中 haDispatchQueuedTasks 持有者为 B、haExpireTasksByTtl/haRecoverStaleTasks 持有者为 A、locked_at 实时刷新。SSE 跨实例断线重连完整链路待 mock LLM 后补做（附录第 5 项）。

### 容量基线首组数据

顺序冒烟 300 请求（交替 /api/health 与 /api/projects）**0 失败**：Avg 12.4ms / P50 16ms / P95 26ms / Max 52ms——印证 CR §9.4 "Web 层不是瓶颈"。并发/SSE 场景待补（见报告待补项）。

### 其他

- 12.9 真实回测遵用户指示跳过（EvalRunner --live 就绪待确认执行）
- CI compose 校验占位值集合已在 v8.9.1 维护约定下保持同步
